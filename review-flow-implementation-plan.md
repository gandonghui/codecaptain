...
Build an end-to-end CodeCaptain review handoff flow that lets one session implement changes and another normal session review them, with CodeCaptain metadata connecting the two sessions invisibly.
...
Do not use `/review`, because that overlaps with CodeCaptain-core's default review command semantics.
...
This is an implementation detail, not a risk. The prompt should be specific enough that the review agent can judge intent and implementation without needing private CodeCaptain routing context.
...
- `opencodeClient.createSession(...)` calls `client.session.create(...)` using the legacy CodeCaptain-core session API.
- CodeCaptain-core supports `metadata` on that API, but CodeCaptain currently only forwards `parentID` and `title`.
- `opencodeClient.updateSession(...)` currently only forwards `title` and `time.archived`.
- CodeCaptain-core `metadata` update replaces the whole metadata object. It does not deep-merge.
...
- If original metadata already has `codecaptain.reviewSessionID`, reuse that session instead of creating a new review session.
...
```ts
type CodeCaptainSessionMetadata = {
  codecaptain?: {
    kind?: 'review'
    originalSessionID?: string
    reviewSessionID?: string
...
- `getCodeCaptainMetadata(session)`
...
### 2. Add Metadata Support To CodeCaptain-core Client Wrapper
...
### 3. Make Session Types Metadata-Aware In CodeCaptain

CodeCaptain-core SDK response types should include metadata in the current v2 SDK legacy `Session`, but verify local imports and generated types used by CodeCaptain.
...
```txt
Produce a review handoff for another agent. Do not compact or mutate session history. Your output is an assistant message that CodeCaptain will send to a separate reviewer agent.
...
- Do not mention CodeCaptain metadata, linked sessions, session IDs, or routing
...
### 7. Register The New CodeCaptain Slash Command

There are two possible implementation paths. Pick the one matching how CodeCaptain-owned commands are currently registered.
...
The command should appear as `/handoff-review` in CodeCaptain command autocomplete.

It should be treated as an CodeCaptain flow command, not only a raw CodeCaptain-core command, because after the handoff assistant output completes CodeCaptain must create/reuse/open/send to the review session.

Implementation options:

1. Intercept `/handoff-review` in `routeMessage(...)` before normal CodeCaptain-core command lookup.
2. Add it to the command store as an CodeCaptain-owned command with a handler.
...
2. Read `original.metadata.codecaptain.reviewSessionID`.
3. If it exists:
   - Try to get that review session in the same directory.
   - If it exists and has `metadata.codecaptain.kind === 'review'`, reuse it.
...
- In an original session with `metadata.codecaptain.reviewSessionID`: show button to send implementation response to the review session.
...
4. Confirm original metadata has `codecaptain.reviewSessionID`.
5. Confirm review metadata has `codecaptain.kind === 'review'` and `codecaptain.originalSessionID`.
...
- Do not change CodeCaptain-core core or SDK unless CodeCaptain cannot access metadata from the existing SDK types.
