# <picture><source media="(prefers-color-scheme: dark)" srcset="docs/references/badges/codecaptain-logo-dark.svg"><img src="docs/references/badges/codecaptain-logo-light.svg" width="32" height="32" align="absmiddle" /></picture> CodeCaptain
...
[![Created with CodeCaptain-core](docs/references/badges/created-with-opencode.svg)](https://opencode.ai)
...
## **CodeCaptain-core, everywhere.** Desktop. Browser. Phone.

### A rich interface for [CodeCaptain-core](https://opencode.ai). Review diffs, manage agents, run dev servers, and keep the big picture while your AI codes.

![CodeCaptain Chat](docs/references/chat_example.png)
...
- **Remote access**: Use CodeCaptain-core from anywhere via browser
...
- Native notifications for task alerts while CodeCaptain is hidden
- One-click open in VS Code, Cursor, Terminal, Finder, Explorer, and more
- Desktop host switcher for local and remote CodeCaptain instances
- Convenient tunnel management without manual setup
- Deep-link connections for joining remote CodeCaptain from a link
...
- **Connect to remote machines** - Desktop app connects to remote CodeCaptain instances over SSH, with dedicated lifecycle and UX flows.

## Quick Start

> **Prerequisite:** [CodeCaptain-core CLI](https://opencode.ai) installed.

### **Desktop (macOS + Windows)**
Download from [Releases](https://github.com/btriapitsyn/codecaptain/releases).

### **VS Code**
Install from [Marketplace](https://marketplace.visualstudio.com/items?itemName=fedaykindev.codecaptain) or search "CodeCaptain" in Extensions.
...
codecaptain update                   # Update to latest
```

`startup enable` snapshots your current environment into the native service so startup behaves like you launched `codecaptain` from the same shell. This preserves provider tokens, PATH, SSH agent settings, and other CLI auth/config env vars. Use `--no-env-snapshot` if you want a minimal service env.

Connect to an existing CodeCaptain-core server:
```bash
...
Bind managed CodeCaptain-core server to all interfaces (use only on trusted networks):
```bash
CODECAPTAIN_OPENCODE_HOSTNAME=0.0.0.0 codecaptain --port 3000
```

Expose CodeCaptain itself on your LAN:
```bash
codecaptain --lan --port 3000 --ui-password secret
```

Add this server to CodeCaptain Desktop or another CodeCaptain app:
```bash
codecaptain connect-url --port 3000 --qr
```

If no CodeCaptain server is running on that port, `connect-url` starts one before generating the link.

Headless/API-only setup for a remote machine:
```bash
codecaptain connect-url --port 3000 --api-only --lan --server http://your-host-or-ip:3000 --qr --ui-password secret
```

This runs CodeCaptain as an API-only server without the desktop app or browser UI assets on that machine, then creates a link for Desktop to import. `--lan` makes the server reachable from other machines. `--server` is the address Desktop should use.

When CodeCaptain was started with `--lan` or `--host 0.0.0.0`, `connect-url` automatically uses a detected LAN IP instead of `127.0.0.1`. Use `--server http://host:3000` to override the advertised address, and include `--lan` when `connect-url` needs to start the server for LAN access.

Paste the printed `codecaptain://connect?...` link in Desktop under Settings -> Remote Instances -> Direct Instances -> Import Link. The link contains the server URL and a client token. It does not enable browser UI password protection; use `--ui-password` when exposing a server beyond localhost.
...
Run CodeCaptain and CodeCaptain-core as separate persistent services — useful when you want to access your
dev machine over a VPN (e.g. Tailscale) or LAN without a Cloudflare tunnel.

**How it works:**
- CodeCaptain-core runs as its own service, binding only to `localhost`.
- CodeCaptain connects to it via `OPENCODE_HOST` and `--lan` makes it reachable on your VPN IP.
...
CodeCaptain will be reachable at `http://<your-vpn-hostname>:3000` from any device on your VPN.
...
> `CODECAPTAIN_HOST=<ip>` to bind to a specific interface instead.
...
Managed-local path note: `CODECAPTAIN_TUNNEL_CONFIG` must point to a path inside the container user home (`/home/codecaptain/...`). If your Cloudflare config references a credentials JSON file, that file path must also be accessible inside the container (mount with `volumes`).
...
- CodeCaptain supports one active tunnel per running instance (port).
...
- Custom themes via JSON files in `~/.config/codecaptain/themes/` - hot reload, no restart
...
- Custom CodeCaptain-core plugins/tools built-in catalog
- Linear integration
- Built-in browser for running dev apps with agent integration

## Acknowledgments

Independent project, not affiliated with the CodeCaptain-core team.

**Special thanks to:**

- [CodeCaptain-core](https://opencode.ai) - For the excellent API and extensible architecture.
