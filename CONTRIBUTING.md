# Contributing to CodeCaptain

## Getting Started

```bash
git clone https://github.com/btriapitsyn/codecaptain.git
cd codecaptain
bun install
```
...
Both are configurable via env vars: `CODECAPTAIN_PORT`, `CODECAPTAIN_HMR_UI_PORT`, `CODECAPTAIN_HMR_API_PORT`.
...
`bun run vscode:dev` opens an Extension Development Host automatically. You can override the editor or workspace with `CODECAPTAIN_VSCODE_BIN` and `CODECAPTAIN_VSCODE_DEV_WORKSPACE`.

Example: `CODECAPTAIN_VSCODE_BIN=cursor bun run vscode:dev`.
...
| `bun run pack:web` | Create a package archive for `@codecaptain/web` |
...
Open an [issue](https://github.com/btriapitsyn/codecaptain/issues) or ask in [Discord](https://discord.gg/ZYRSdnwwKA).
