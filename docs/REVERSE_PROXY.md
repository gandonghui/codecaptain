...
Use this guide when running CodeCaptain behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm CodeCaptain works directly first.
...
- Disable gzip on the proxy if CodeCaptain is already compressing responses.
...
- CodeCaptain reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|codecaptain/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
location = /api/codecaptain/events {
    proxy_pass http://127.0.0.1:3000;
    proxy_set_header Accept "text/event-stream";
    proxy_set_header Cache-Control "no-cache";
    proxy_buffering off;
...
- CodeCaptain compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by CodeCaptain, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
