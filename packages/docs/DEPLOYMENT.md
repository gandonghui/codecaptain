...
Rendering and hosting still happen in `codecaptain-website` (`apps/docs`).
...
- creates `codecaptain-docs-source-<sha>.tar.gz`
...
to `codecaptain-website` so it re-syncs and redeploys the docs. This fires on
...
- `CODECAPTAIN_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  `codecaptain/codecaptain-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `codecaptain` (`main` on a
  push, the tag on a release)
- `archive_name`

`codecaptain-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`codecaptain` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `CODECAPTAIN_REPO_TOKEN`
