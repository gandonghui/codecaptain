---
title: Reverse proxy
description: Skonfiguruj CodeCaptain poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz CodeCaptain za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że CodeCaptain działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli CodeCaptain już kompresuje odpowiedzi.
...
- CodeCaptain dostępny bezpośrednio w LAN
...
- CodeCaptain kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez CodeCaptain, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
