---
title: Proxy inverso
description: Configura CodeCaptain correctamente detrás de Nginx, Nginx Proxy Manager u otro proxy inverso.
---

# Proxy inverso

Usa esta página si ejecutas CodeCaptain detrás de Nginx, Nginx Proxy Manager, Caddy, Cloudflare u otro proxy inverso.

## Antes de ponerle un proxy

1. Confirma primero que CodeCaptain funciona directamente.
...
- Desactiva gzip en el proxy si CodeCaptain ya comprime las respuestas.
...
- CodeCaptain accesible directamente en la LAN
...
- CodeCaptain comprime las respuestas HTTP con gzip (umbral de 1 KB).
...
Las rutas de streaming SSE están excluidas de la compresión por CodeCaptain, pero la CDN aún puede almacenarlas en búfer. Consulta la documentación de tu CDN para saber cómo desactivar el búfer en las rutas SSE.
