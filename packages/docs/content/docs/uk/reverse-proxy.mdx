...
description: Налаштуйте CodeCaptain коректно за Nginx, Nginx Proxy Manager або іншим зворотним проксі.
---

# Зворотний проксі

Скористайтеся цією сторінкою, якщо запускаєте CodeCaptain за Nginx, Nginx Proxy Manager, Caddy, Cloudflare або іншим зворотним проксі.

## Перш ніж проксувати

1. Спершу переконайтеся, що CodeCaptain працює напряму.
...
- Вимкніть gzip на проксі, якщо CodeCaptain уже стискає відповіді.
...
- CodeCaptain доступний напряму в LAN
...
- CodeCaptain стискає HTTP-відповіді через gzip (поріг 1 КБ).
...
SSE-маршрути потоків виключені зі стиснення в CodeCaptain, але CDN усе одно може їх буферизувати. Перевірте документацію свого CDN щодо вимкнення буферизації на SSE-шляхах.
