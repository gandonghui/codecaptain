...
Uma tarefa agendada executa um prompt para você em uma agenda — por exemplo, um "resumir as mudanças de ontem" diário ou uma limpeza semanal. Quando ela é executada, o CodeCaptain inicia uma nova sessão e envia o prompt sozinho. Abra o agendador pelo botão no topo da barra lateral de sessões.
...
As tarefas só disparam enquanto o servidor do CodeCaptain estiver em execução. Se você o fechar, as execuções agendadas ficam pausadas até ele voltar a funcionar.
