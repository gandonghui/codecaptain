...
const getDraftKey = (sessionId: string | null): string =>
    `codecaptain_chat_input_draft_${sessionId ?? 'new'}`;
...
const getConfirmedMentionsKey = (sessionId: string | null): string =>
    `codecaptain_chat_confirmed_mentions_${sessionId ?? 'new'}`;
...
const ChatInputComponent: React.FC<ChatInputProps> = ({ onOpenSettings, scrollToBottom }) => {
    ...
    const hasDraggedFiles = React.useCallback((dataTransfer: DataTransfer | null | undefined): boolean => {
        ...
        if (dataTransfer.types) {
            ...
            if (lowerTypes.includes('application/x-codecaptain-file-path')) return true;
    ...
    const handleDragEnter = (e: React.DragEvent) => {
        ...
        const isInternal = e.dataTransfer.types?.includes('application/x-codecaptain-file-path') ?? false;
    ...
    const handleDrop = async (e: React.DragEvent) => {
        ...
        const internalPath = e.dataTransfer.getData('application/x-codecaptain-file-path');
