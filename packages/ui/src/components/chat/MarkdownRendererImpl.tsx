...
const useExternalLinkInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (anchor.getAttribute('data-codecaptain-file-link') === 'true') {
        return;
      }
...
const FILE_LINK_SELECTOR = '[data-codecaptain-file-link="true"]';
const BLOCK_PATH_TOKEN_ATTR = 'data-codecaptain-block-path-token';
const BLOCK_PATH_TOKEN_SELECTOR = `[${BLOCK_PATH_TOKEN_ATTR}]`;
const CODE_BLOCK_PATH_SCANNED_ATTR = 'data-codecaptain-block-paths-scanned';
...
// `data-codecaptain-block-path-token`. `annotateFileLinks` then promotes those
...
// `data-codecaptain-block-paths-scanned` once processed so the walk is not
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const clearFileLinkAttributes = (candidate: HTMLElement) => {
      candidate.removeAttribute('data-codecaptain-file-link');
      candidate.removeAttribute('data-codecaptain-file-ref');
      candidate.removeAttribute('data-codecaptain-file-path');
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void existsPromise.then((exists) => {
          ...
          candidate.setAttribute('data-codecaptain-file-link', 'true');
          candidate.setAttribute('data-codecaptain-file-ref', latestRawCandidate);
          candidate.setAttribute('data-codecaptain-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = async (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-codecaptain-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-codecaptain-file-link') !== 'true') {
        return;
      }
