...
import { CODECAPTAIN_PROJECT_NOTES_MAX_LENGTH, getProjectNotesAndTodos, saveProjectNotesAndTodos } from '@/lib/codecaptainConfig';
...
const appendDistilledInsightToNotes = (existingNotes: string, insight: string): string => {
  const trimmedInsight = insight.trim().replace(/^[-*+]\s+/, '').slice(0, CODECAPTAIN_PROJECT_NOTES_MAX_LENGTH);
  if (!trimmedInsight) {
...
export const TextSelectionMenu: React.FC<TextSelectionMenuProps> = ({ containerRef }) => {
  ...
  const handleAddToNotes = React.useCallback(async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('codecaptain:project-notes-updated', {
