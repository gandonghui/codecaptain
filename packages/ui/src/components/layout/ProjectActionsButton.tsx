...
const AUTO_DISCOVER_ACTION_ID = '__codecaptain_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<CodeCaptainProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<CodeCaptainProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: CodeCaptainProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: CodeCaptainProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<CodeCaptainProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: CodeCaptainProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: CodeCaptainProjectAction, toggleStopIfRunning = false) => {
