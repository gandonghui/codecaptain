...
const compactPath = (value: string | null | undefined): string => {
  ...
  if (!path) return '';
  const home = typeof window !== 'undefined' ? window.__CODECAPTAIN_HOME__ : '';
...
const MiniChatHeader: React.FC<{ mode: MiniChatMode }> = ({ mode }) => {
  ...
  const macosMajor = typeof window !== 'undefined' ? window.__CODECAPTAIN_MACOS_MAJOR__ ?? 0 : 0;
  const hasMacTrafficLights = Number.isFinite(macosMajor) && macosMajor > 0;
  const isWindowsElectronDesktop = typeof window !== 'undefined'
    && Boolean(window.__CODECAPTAIN_ELECTRON__)
    && window.__CODECAPTAIN_PLATFORM__ === 'win32';
  ...
  const projectLabel = React.useMemo(() => {
    ...
    if (!project) return directoryLabel || 'CodeCaptain';
