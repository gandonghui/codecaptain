...
import { parseProjectPlanMarkdown } from '@/lib/codecaptainConfig';
...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'codecaptain:plan:md-viewer-mode';
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewCodeCaptainSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, currentProjectRef, initializeNewCodeCaptainSession, pendingPlanSend, resolvedPath, routeToChat, sendMessage, sendPromptTitle, setCurrentSession]
  );
