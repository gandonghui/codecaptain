...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({ visible }: WindowsWindowControlsProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('codecaptain:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('codecaptain:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
