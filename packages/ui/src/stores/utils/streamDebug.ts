...
export const sessionStatusDebugEnabled = (): boolean => {
    if (typeof window === 'undefined') return false;
    try {
        return window.localStorage.getItem('codecaptain_session_status_debug') === '1';
    } catch {
        return false;
    }
};

const STREAM_PERF_STORAGE_KEY = 'codecaptain_stream_perf';
...
declare global {
    interface Window {
        __codecaptainStreamPerfState?: StreamPerfState;
        __codecaptainVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__codecaptainStreamPerfState) {
        const startedAt = Date.now();
        window.__codecaptainStreamPerfState = {
    ...
    return window.__codecaptainStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__codecaptainStreamPerfState;
        delete window.__codecaptainVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__codecaptainVsCodeStreamPerfState) {
        window.__codecaptainVsCodeStreamPerfState = {
            ...window.__codecaptainVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__codecaptainStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__codecaptainVsCodeStreamPerfState;
    if (!streamPerfEnabled() || !state) {
        return {
            enabled: false,
            startedAt: null,
