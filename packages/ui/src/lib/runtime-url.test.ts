...
describe('createRuntimeUrlResolver', () => {
  ...
  test('uses realtime base URL for SSE and WebSocket URLs', () => {
    ...
    expect(urls.sse('/api/codecaptain/events')).toBe('https://realtime.example/api/codecaptain/events');
  ...
  test('uses injected desktop API base URL for packaged WebSocket URLs', () => {
    withWindow({
      location: { origin: 'codecaptain-ui://app', href: 'codecaptain-ui://app/index.html' },
      __CODECAPTAIN_API_BASE_URL__: 'http://127.0.0.1:57123',
    }, () => {
      const urls = createRuntimeUrlResolver({});
  ...
  test('reads injected desktop API base URL at call time', () => {
    withWindow({
      ...
      const urls = createRuntimeUrlResolver({});
      (window as typeof window & { __CODECAPTAIN_API_BASE_URL__?: string }).__CODECAPTAIN_API_BASE_URL__ = 'http://127.0.0.1:57123';
  ...
  test('adds short-lived URL auth query to realtime and authenticated asset URLs only', () => {
    ...
    try {
      ...
      expect(urls.sse('/api/codecaptain/events')).toBe(
        'https://api.example/api/codecaptain/events?oc_url_token=oc_url_secret',
      );
      expect(urls.websocket('/api/global/event/ws', { lastEventId: 'evt-1' })).toBe(
  ...
  test('does not put the long-lived client token in URLs', () => {
    ...
    try {
      const urls = createRuntimeUrlResolver({ apiBaseUrl: 'https://api.example' });
      expect(urls.sse('/api/codecaptain/events')).toBe('https://api.example/api/codecaptain/events');
