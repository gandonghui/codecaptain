...
export const applyPersistedHomeDirectoryToWindow = (homeDirectory: string): void => {
  ...
  if (typeof window.__CODECAPTAIN_HOME__ === 'string' && window.__CODECAPTAIN_HOME__.length > 0) {
    return;
  }

  try {
    window.__CODECAPTAIN_HOME__ = homeDirectory;
  } catch {
...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('codecaptain.pwaName', normalized);
    } else {
      localStorage.removeItem('codecaptain.pwaName');
    }
  }
...
const dispatchSettingsSynced = (settings: DesktopSettings): void => {
  ...
  window.dispatchEvent(new CustomEvent<DesktopSettings>('codecaptain:settings-synced', { detail: settings }));
};
