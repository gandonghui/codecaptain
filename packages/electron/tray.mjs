...
//
// Surfaces a glanceable, always-visible view of CodeCaptain's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'CodeCaptain — no active sessions';
  const bits = [];
  ...
  return `CodeCaptain — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    template.push(
      ...
      { label: 'Show CodeCaptain', click: () => onAction({ type: 'show-main-window' }) },
      { type: 'separator' },
      { label: 'Quit CodeCaptain', click: () => onAction({ type: 'quit' }) },
    );
