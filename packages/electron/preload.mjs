...
const localOrigin = readArgValue('--codecaptain-local-origin');
const apiBaseUrl = readArgValue('--codecaptain-api-base-url');
const clientToken = readArgValue('--codecaptain-client-token');
const homeDirectory = readArgValue('--codecaptain-home');
const macosMajorRaw = readArgValue('--codecaptain-macos-major');
...
const hasMacVibrancy = macVibrancySupported && readArgValue('--codecaptain-mac-vibrancy') !== '0';
...
//  - __CODECAPTAIN_ELECTRON__ is a shell-identity flag (no capability).
...
//  - __CODECAPTAIN_DESKTOP__ is the IPC channel to the main process. It is
...
const isLocalPage = currentOrigin !== 'null'
  && (currentOrigin === 'codecaptain-ui://app'
  || (localOrigin && currentOrigin === localOrigin));

// Remote pages need __CODECAPTAIN_LOCAL_ORIGIN__ so the HostSwitcher knows
...
if (localOrigin) {
  contextBridge.exposeInMainWorld('__CODECAPTAIN_LOCAL_ORIGIN__', localOrigin);
}

if (apiBaseUrl) {
  contextBridge.exposeInMainWorld('__CODECAPTAIN_API_BASE_URL__', apiBaseUrl);
}

if (clientToken && isLocalPage) {
  contextBridge.exposeInMainWorld('__CODECAPTAIN_CLIENT_TOKEN__', clientToken);
}
...
if (isLocalPage && homeDirectory) {
  contextBridge.exposeInMainWorld('__CODECAPTAIN_HOME__', homeDirectory);
}
...
if (Number.isFinite(macosMajor) && macosMajor > 0) {
  contextBridge.exposeInMainWorld('__CODECAPTAIN_MACOS_MAJOR__', macosMajor);
}

contextBridge.exposeInMainWorld('__CODECAPTAIN_ELECTRON__', {
  runtime: 'electron',
  macVibrancy: hasMacVibrancy,
  macVibrancySupported,
});

contextBridge.exposeInMainWorld('__CODECAPTAIN_PLATFORM__', process.platform);
...
ipcRenderer.on('codecaptain:emit', (_evt, payload) => {
  ...
  if (event === 'codecaptain:vibrancy-ready') {
...
// ipcMain.handle('codecaptain:invoke') decides per-command what is safe
// for non-local callers (window/host-switcher ops yes, file/shell ops
// no). See COMMANDS_SAFE_FOR_REMOTE in main.mjs.
contextBridge.exposeInMainWorld('__CODECAPTAIN_DESKTOP__', {
  invoke: (cmd, args) => ipcRenderer.invoke('codecaptain:invoke', cmd, args || {}),
  openDialog: (options) => ipcRenderer.invoke('codecaptain:dialog:open', options || {}),
  grantFileAccess: (filePath) => ipcRenderer.invoke('codecaptain:file:grant-existing', filePath),
  openExternal: (url) => ipcRenderer.invoke('codecaptain:invoke', 'desktop_open_external_url', { url }),
