...
Electron desktop runtime for CodeCaptain on macOS and Windows.

This package owns the native shell: windows, menus, deep links, native notifications, auto-updates, host switching, SSH connections, tunnel helpers, and packaged desktop builds. The web UI and CodeCaptain server logic still live in `packages/web` and shared React UI lives in `packages/ui`.

## How It Runs

Desktop starts the CodeCaptain web server in the same Electron main process. There is no separate sidecar subprocess for the CodeCaptain server.

`main.mjs` imports `@codecaptain/web/server/index.js` and calls `startWebUiServer()`. The Electron window then loads the UI from the local server in development, or from packaged `resources/web-dist` assets in packaged builds.

The preload bridge exposes desktop-only APIs to the web UI through `window.__CODECAPTAIN_DESKTOP__`. Privileged commands are checked in `main.mjs`, not only in the UI.
...
| Variable | Use |
|----------|-----|
| `CODECAPTAIN_ELECTRON_DEV=1` | Marks the runtime as desktop development mode |
| `CODECAPTAIN_ELECTRON_USE_BUNDLED_UI=1` | Uses staged web assets instead of the HMR dev server |
| `CODECAPTAIN_HMR_UI_PORT` | Preferred Vite UI port for desktop dev, default `5173` |
| `CODECAPTAIN_HMR_API_PORT` | Preferred API port for desktop dev, default `3901` |
| `CODECAPTAIN_RUNTIME=desktop` | Set by Electron before starting the web server |
| `CODECAPTAIN_DESKTOP_NOTIFY=true` | Enables desktop notification flow in the web server |
| `CODECAPTAIN_SKIP_API_COMPRESSION=true` | Defaulted by Desktop to reduce local CPU overhead |
| `OPENCODE_HOST` / `OPENCODE_PORT` / `OPENCODE_SKIP_START` | Connect Desktop to an external CodeCaptain-core server instead of starting one locally |
...
2. Add the real command handling in `main.mjs` under `codecaptain:invoke`.
...
Electron uses `electron-log`. In development, console logs are also visible in the terminal. In packaged apps, logs are written through the platform log path for the `CodeCaptain` app name.

Development builds use a separate user data directory named `CodeCaptain Dev`, so dev state does not overwrite normal packaged app state.

## Things To Be Careful With

- Keep desktop-specific code in this package. Do not move CodeCaptain-core feature backend logic into Electron.
- Use hidden Windows process launches for background helpers. Avoid visible console flashes.
- Keep `@codecaptain/web`, `bun-pty`, `node-pty`, and native modules external in `bundle-main.mjs`; bundling them can break Electron startup.
