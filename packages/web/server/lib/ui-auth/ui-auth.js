...
const RATE_LIMIT_MAX_ATTEMPTS = Number(process.env.CODECAPTAIN_RATE_LIMIT_MAX_ATTEMPTS) || 10;
const RATE_LIMIT_LOCKOUT_MS = 15 * 60 * 1000;
const RATE_LIMIT_CLEANUP_MS = 60 * 60 * 1000;
const RATE_LIMIT_NO_IP_MAX_ATTEMPTS = Number(process.env.CODECAPTAIN_RATE_LIMIT_NO_IP_MAX_ATTEMPTS) || 3;
...
const isUrlAuthReadableHttpPath = (pathname) => {
  return pathname === '/api/event'
    ...
    || pathname === '/api/codecaptain/events'
...
const JWT_SECRET_FILE = path.join(CODECAPTAIN_DATA_DIR, 'jwt-secret');
...
function getOrCreateJwtSecret() {
  ...
  try {
    fs.mkdirSync(CODECAPTAIN_DATA_DIR, { recursive: true });
