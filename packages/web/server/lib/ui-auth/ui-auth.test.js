...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codecaptain-ui-auth-test-'));
process.env.CODECAPTAIN_DATA_DIR = dataDir;
...
describe('ui auth client credential seam', () => {
  ...
  it('issues desktop client tokens with the UI session expiry', async () => {
    ...
    expect(createClientInput.label).toBe('CodeCaptain Desktop');
