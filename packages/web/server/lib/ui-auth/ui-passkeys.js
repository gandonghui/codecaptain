...
const DEFAULT_RP_NAME = 'CodeCaptain';
...
const PASSKEY_STORE_FILE = path.join(CODECAPTAIN_DATA_DIR, 'ui-passkeys.json');
...
export const createUiPasskeys = ({
  ...
  const beginRegistration = async (req, { label } = {}) => {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID,
      userID,
      userName: 'codecaptain-ui',
      userDisplayName: 'CodeCaptain UI',
  ...
  const finishAuthentication = async (payload) => {
    ...
    if (!passkey) {
      const error = new Error('That passkey is not registered for this CodeCaptain instance');
      error.statusCode = 404;
      throw error;
    }
