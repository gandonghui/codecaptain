...
const PACKAGE_NAME = '@codecaptain/web';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const CHANGELOG_URL = 'https://raw.githubusercontent.com/btriapitsyn/codecaptain/main/CHANGELOG.md';
let cachedDetectedPm = null;

function getSpawnSyncBaseOptions() {
  return process.platform === 'win32' ? { windowsHide: true } : {};
}
const UPDATE_CHECK_URL = process.env.CODECAPTAIN_UPDATE_API_URL || 'https://api.codecaptain.dev/v1/update/check';

function getCodeCaptainConfigDir() {
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, 'codecaptain');
  }
...
function getOrCreateInstallId(scope = 'web') {
  const configDir = getCodeCaptainConfigDir();
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'codecaptain.cmd' : 'codecaptain';
...
export function detectPackageManagerDetails() {
  ...
  const forcedPm = process.env.CODECAPTAIN_PACKAGE_MANAGER?.trim();
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('codecaptain');
  } catch {
    return false;
  }
}
  ...
  if (currentVersion !== 'unknown') {
    ...
    if (remote) {
      ...
      return {
        ...remote,
        packageManager: pm,
        updateCommand: 'codecaptain update',
      };
    }
  }
