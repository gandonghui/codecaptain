...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const resolveOpencodeCliPath = () => {
    const explicit = [
      ...
      process.env.CODECAPTAIN_OPENCODE_PATH,
      process.env.CODECAPTAIN_OPENCODE_BIN,
    ]
  ...
  const resolveNodeCliPath = () => {
    const explicit = [process.env.NODE_BINARY, process.env.CODECAPTAIN_NODE_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const resolveBunCliPath = () => {
    const explicit = [process.env.BUN_BINARY, process.env.CODECAPTAIN_BUN_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const createConfiguredOpencodeBinaryError = (raw, normalized) => {
    ...
    const messageSuffix = 'CodeCaptain needs the standalone opencode CLI. Install it and set settings.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
    const error = (() => {
  ...
  const createConfiguredWslOpencodeError = (raw) => new Error(
    `Configured settings.opencodeBinary uses WSL but CodeCaptain could not resolve a WSL OpenCode command: ${raw}. Ensure WSL is available and opencode is installed in the configured distro.`
  );
  ...
  const applyOpencodeBinaryFromSettings = async (options = {}) => {
    ...
    try {
      ...
      if (explicitWslPath && explicitWslPath[1] && explicitWslPath[1].trim().length > 0) {
        ...
        console.warn(`Configured settings.opencodeBinary uses WSL, which is no longer supported by CodeCaptain desktop: ${raw}`);
        return null;
      }
      ...
      if (process.platform === 'win32' && (isWslExecutableValue(raw) || isWslExecutableValue(normalized || ''))) {
        ...
        console.warn(`Configured settings.opencodeBinary points to WSL, which is no longer supported by CodeCaptain desktop: ${raw}`);
        return null;
      }
  ...
  const resolveGitBinaryForSpawn = () => {
    ...
    const explicit = [process.env.GIT_BINARY, process.env.CODECAPTAIN_GIT_BINARY]
