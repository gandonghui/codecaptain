...
const originalCodeCaptainWslBinary = process.env.CODECAPTAIN_WSL_BINARY;
...
afterEach(() => {
  ...
  if (typeof originalCodeCaptainWslBinary === 'string') {
    process.env.CODECAPTAIN_WSL_BINARY = originalCodeCaptainWslBinary;
  } else {
    delete process.env.CODECAPTAIN_WSL_BINARY;
  }
});
...
describe('OpenCode env runtime', () => {
  ...
  it('throws a specific error for a configured directory without an executable CLI in strict mode', async () => {
    const dir = createTempDir('codecaptain-opencode-dir-');
  ...
  it('applies a valid configured executable OpenCode binary', async () => {
    const dir = createTempDir('codecaptain-opencode-bin-');
  ...
  it('rejects WSL settings in strict mode', async () => {
    setPlatform('win32');
    const dir = createTempDir('codecaptain-no-wsl-');
    process.env.PATH = dir;
    process.env.SystemRoot = dir;
    process.env.WSL_BINARY = path.join(dir, 'missing-wsl.exe');
    process.env.CODECAPTAIN_WSL_BINARY = path.join(dir, 'missing-codecaptain-wsl.exe');
  ...
  it('does not auto-detect OpenCode from WSL fallback paths', () => {
    setPlatform('win32');
    const dir = createTempDir('codecaptain-wsl-opencode-');
  ...
  it('launches Windows cmd shims through cmd call without embedded quotes', () => {
    ...
    const dir = createTempDir('codecaptain-opencode-cmd-');
  ...
  it('resolves npm OpenCode cmd shims to the packaged Windows executable', () => {
    setPlatform('win32');
    const npmDir = createTempDir('codecaptain-opencode-npm-');
