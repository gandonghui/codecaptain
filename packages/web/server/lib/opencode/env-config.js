export const resolveOpenCodeEnvConfig = (options = {}) => {
  ...
  const configuredOpenCodePort = (() => {
    const raw =
      env.OPENCODE_PORT ||
      env.CODECAPTAIN_OPENCODE_PORT ||
      env.CODECAPTAIN_INTERNAL_PORT;
    if (!raw) {
      return null;
    }
  ...
  const configuredOpenCodeHostname = (() => {
    const raw = env.CODECAPTAIN_OPENCODE_HOSTNAME;
    ...
    if (!trimmed) {
      logger.warn(
        `[config] Ignoring CODECAPTAIN_OPENCODE_HOSTNAME=${JSON.stringify(raw)}: empty after trimming`,
      );
      return '127.0.0.1';
    }
    return trimmed;
