# CodeCaptain-core Module Documentation

## Purpose
This module provides CodeCaptain-core server integration utilities for the web server runtime, including configuration management and provider authentication.
...
- `packages/web/server/lib/opencode/auth-state-runtime.js`: managed CodeCaptain-core server auth password/header runtime.
...
- `packages/web/server/lib/opencode/routes.js`: CodeCaptain-core/provider settings and auth-related route registration.
- `packages/web/server/lib/opencode/lifecycle.js`: CodeCaptain-core process lifecycle runtime (startup, restart, readiness, health monitoring).
- `packages/web/server/lib/opencode/env-runtime.js`: CodeCaptain-core CLI/binary resolution and shell environment runtime.
- `packages/web/server/lib/opencode/env-config.js`: CodeCaptain-core-related environment variable parsing and validation (host/port/hostname).
...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/CodeCaptain route wiring.
- `packages/web/server/lib/opencode/network-runtime.js`: CodeCaptain-core URL construction, health-probe readiness checks, and API prefix runtime.
...
- `packages/web/server/lib/opencode/opencode-resolution-runtime.js`: CodeCaptain-core binary resolution snapshot runtime for settings routes and diagnostics.
...
- `packages/web/server/lib/opencode/server-utils-runtime.js`: shared server runtime utilities for CodeCaptain-core proxy wiring, CodeCaptain-core port/readiness helpers, and snapshot fetchers.
- `packages/web/server/lib/opencode/codecaptain-routes.js`: CodeCaptain update and models metadata route registration.
...
- `packages/web/server/lib/opencode/proxy.js`: CodeCaptain-core API/SSE forwarding and readiness-gate route registration.
- `packages/web/server/lib/opencode/session-runtime.js`: session status/attention/activity runtime for CodeCaptain-core SSE events.
...
- `packages/web/server/lib/ui-auth/ui-auth.js`: UI session authentication runtime (outside CodeCaptain-core module).
- `packages/web/server/lib/ui-auth/ui-passkeys.js`: UI passkey storage and WebAuthn registration/authentication helpers (outside CodeCaptain-core module).
...
- `OPENCODE_DATA_DIR`: CodeCaptain-core data directory path constant.
...
- `ensureDirs()`: Creates required CodeCaptain-core directories.
...
- `registerCodeCaptain-coreRoutes(app, dependencies)`: Registers CodeCaptain-core-owned HTTP routes and internal module runtime:
  - `GET /api/config/settings`
  - `PUT /api/config/settings`
  - `GET /api/config/opencode-resolution`
  - `POST /api/opencode/upgrade` (proxies CodeCaptain-core upgrade, then restarts managed CodeCaptain-core so the new binary is active)
...
- Returned API:
  - `processCodeCaptain-coreSsePayload(payload)`
...
- `createCodeCaptain-coreLifecycleRuntime(dependencies)`: creates lifecycle runtime for managed/external CodeCaptain-core process orchestration.
- Returned API:
  - `startCodeCaptain-core()`
  - `restartCodeCaptain-core()`
  - `waitForCodeCaptain-coreReady(timeoutMs?, intervalMs?)`
  - `waitForAgentPresence(agentName, timeoutMs?, intervalMs?)`
  - `refreshCodeCaptain-coreAfterConfigChange(reason, options?)`
  - `bootstrapCodeCaptain-coreAtStartup()`
  - `startHealthMonitoring(healthCheckIntervalMs)`
  - `waitForPortRelease(port, timeoutMs, hostname?)`
  - `killProcessOnPort(port)`

## Public exports (env-runtime.js)
- `createCodeCaptain-coreEnvRuntime(dependencies)`: creates runtime that owns CodeCaptain-core CLI environment and binary discovery state.
- Returned API:
  ...
  - `resolveManagedCodeCaptain-coreLaunchSpec(opencodePath)`: resolves the effective managed CodeCaptain-core launch target, unwrapping Windows package-manager shims to a direct native binary or explicit runtime+script when possible.
  - `resolveGitBinaryForSpawn()`
  - `resolveWslExecutablePath()`
  - `buildWslExecArgs(execArgs, distroOverride?)`
  - `isExecutable(filePath)`
  - `searchPathFor(binaryName)`
  - `clearResolvedCodeCaptain-coreBinary()`

## Public exports (env-config.js)
- `resolveCodeCaptain-coreEnvConfig(options?)`: resolves and validates CodeCaptain-core host/port/hostname environment configuration.
- Returned object fields:
  - `configuredCodeCaptain-corePort`
  - `configuredCodeCaptain-coreHost`
  - `effectivePort`
  - `configuredCodeCaptain-coreHostname`
...
- Returned API:
  - `getOrCreateHmrState()`
  - `ensureUserProvidedCodeCaptain-corePassword(hmrState)`
  - `getUserProvidedCodeCaptain-corePassword(hmrState)`
  - `resolveCodeCaptain-coreAuthFromState({ hmrState, userProvidedCodeCaptain-corePassword })`
  - `syncStateFromRuntime(hmrState, runtime)`
  - `restoreRuntimeFromState({ hmrState, userProvidedCodeCaptain-corePassword })`
...
- `createCodeCaptain-coreNetworkRuntime(dependencies)`: creates runtime for CodeCaptain-core network and URL concerns.
- Returned API:
  - `waitForReady(url, timeoutMs?)`
  - `normalizeApiPrefix(prefix)`
  - `setDetectedCodeCaptain-coreApiPrefix()`
  - `buildCodeCaptain-coreUrl(path, prefixOverride?)`
  - `ensureCodeCaptain-coreApiPrefix()`
  - `scheduleCodeCaptain-coreApiDetection()`
...
- `createCodeCaptain-coreAuthStateRuntime(dependencies)`: creates runtime for managed CodeCaptain-core auth password state and request headers.
- Returned API:
  - `getCodeCaptain-coreAuthHeaders()`
  - `isCodeCaptain-coreConnectionSecure()`
  - `ensureLocalCodeCaptain-coreServerPassword(options?)`
...
- `createServerUtilsRuntime(dependencies)`: creates server utility runtime for CodeCaptain-core orchestration helpers.
- Returned API:
  - `setCodeCaptain-corePort(port)`
  - `waitForCodeCaptain-corePort(timeoutMs?)`
  - `buildAugmentedPath()`
...
- `createGracefulShutdownRuntime(dependencies)`: creates graceful shutdown runtime for managed CodeCaptain-core and web server teardown sequencing.
- Returned API:
...
- `createCodeCaptain-coreResolutionRuntime(dependencies)`: creates runtime for CodeCaptain-core binary/source snapshot resolution.
- Returned API:
  - `getCodeCaptain-coreResolutionSnapshot(settings)`: returns configured/resolved CodeCaptain-core binary details plus effective managed-launch fields (`launchBinary`, `launchArgs`, `launchWrapperType`) when applicable.
...
## Public exports (codecaptain-routes.js)
- `registerCodeCaptainRoutes(app, dependencies)`: registers CodeCaptain endpoints:
  - `GET /api/codecaptain/update-check`
  - `POST /api/codecaptain/update-install`
  - `GET /api/codecaptain/models-metadata`
  - `GET /api/zen/models`
...
- `registerCodeCaptain-coreProxy(app, dependencies)`: registers CodeCaptain-core proxy routes and middleware.
- Owns:
  ...
  - CodeCaptain-core readiness gate for proxied `/api` requests

## Public exports (watcher.js)
- `createCodeCaptain-coreWatcherRuntime(dependencies)`: creates global event watcher runtime backed by the shared upstream SSE reader.
- Returned API:
  - `start()`
  - `stop()`
- Behavior:
  - Waits for CodeCaptain-core readiness before attaching the watcher.
...
- Rate limit config: `CODECAPTAIN_RATE_LIMIT_MAX_ATTEMPTS`, `CODECAPTAIN_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.

## Notes for contributors
- This module serves as foundation for CodeCaptain-core-related server utilities.
