export const registerProjectIconRoutes = (app, dependencies) => {
  ...
  const projectIconsDirPath = path.join(codecaptainDataDir, 'project-icons');
  ...
  const applyProjectIconSvgTheme = (svgMarkup, themeVariant, iconColor) => {
    ...
    const overrideStyle = `<style data-codecaptain-theme-icon="1">:root{color:${color}!important;}</style>`;
