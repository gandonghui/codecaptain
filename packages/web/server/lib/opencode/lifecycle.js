...
const HEALTH_CHECK_TIMEOUT_MS = parsePositiveInt(process.env.CODECAPTAIN_OPENCODE_HEALTH_TIMEOUT_MS, 5000);
const HEALTH_CHECK_MAX_CONSECUTIVE_FAILURES = parsePositiveInt(
  process.env.CODECAPTAIN_OPENCODE_HEALTH_CONSECUTIVE_FAILURES,
  20
);
const HEALTH_CHECK_INTERVAL_OVERRIDE_MS = parsePositiveInt(process.env.CODECAPTAIN_OPENCODE_HEALTH_INTERVAL_MS, 0);
const HEALTH_CHECK_RESULT_CACHE_MS = parsePositiveInt(process.env.CODECAPTAIN_OPENCODE_HEALTH_CACHE_MS, 750);
...
export const createOpenCodeLifecycleRuntime = (deps) => {
  ...
  const createManagedOpenCodeServerProcess = async ({ hostname, port, timeout, cwd, env: processEnv, shellEnvKeysCount = 0 }) => {
    ...
    // actual host (Electron sets CODECAPTAIN_RUNTIME='desktop'; the standalone
    ...
    registerManagedProcess({
      ...
      port,
      binary,
      runtime: process.env.CODECAPTAIN_RUNTIME || 'web',
    });
  ...
  const restartOpenCode = async () => {
    ...
    state.currentRestartPromise = (async () => {
      ...
      state.isOpenCodeReady = false;
      state.openCodeNotReadySince = Date.now();
      console.log('Restarting CodeCaptain-core process...');
