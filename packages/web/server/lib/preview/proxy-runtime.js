...
const PREVIEW_BRIDGE_SCRIPT_ID = 'codecaptain-preview-bridge';
...
const PREVIEW_BRIDGE_SCRIPT = String.raw`(() => {
  if (window.__codecaptainPreviewBridgeInstalled) return;
  window.__codecaptainPreviewBridgeInstalled = true;

  const SOURCE = 'codecaptain-preview-bridge';
  const VERSION = 1;
  const MAX_TEXT = 500;
  const MAX_ARG = 1000;
  const TARGET_ORIGIN = typeof window.__codecaptainPreviewTargetOrigin === 'string' ? window.__codecaptainPreviewTargetOrigin : '';
  ...
  const installColorSchemeMatchMediaPatch = () => {
    if (window.__codecaptainPreviewColorSchemePatched || typeof window.matchMedia !== 'function') return;
    window.__codecaptainPreviewColorSchemePatched = true;
  ...
  const applyPreviewColorScheme = (scheme) => {
    ...
    try {
      ...
      root.dataset.codecaptainPreviewColorScheme = next;
  ...
  const installViteHmrProxyPatch = () => {
    if (window.__codecaptainViteHmrProxyPatched || typeof window.WebSocket !== 'function') return;
    window.__codecaptainViteHmrProxyPatched = true;
    ...
    function CodeCaptainPreviewWebSocket(url, protocols) {
      const protocolList = Array.isArray(protocols) ? protocols : [protocols];
    ...
    CodeCaptainPreviewWebSocket.prototype = NativeWebSocket.prototype;
    Object.setPrototypeOf(CodeCaptainPreviewWebSocket, NativeWebSocket);
    Object.defineProperty(CodeCaptainPreviewWebSocket, 'name', { value: 'WebSocket' });
    window.WebSocket = CodeCaptainPreviewWebSocket;
  };

  const installAppRequestProxyPatch = () => {
    if (window.__codecaptainAppRequestProxyPatched) return;
    window.__codecaptainAppRequestProxyPatched = true;
    const proxyMatch = window.location.pathname.match(/^(\/api\/preview\/proxy\/[a-f0-9]{16,64})(?:\/|$)/i);
    if (!proxyMatch) return;
    const proxyBase = proxyMatch[1];
    const currentSearchParams = new URL(window.location.href).searchParams;
    ...
    if (typeof window.EventSource === 'function') {
      ...
      function CodeCaptainPreviewEventSource(url, eventSourceInitDict) {
        return new NativeEventSource(proxiedUrl(String(url)), eventSourceInitDict);
      }
      CodeCaptainPreviewEventSource.prototype = NativeEventSource.prototype;
      Object.setPrototypeOf(CodeCaptainPreviewEventSource, NativeEventSource);
      Object.defineProperty(CodeCaptainPreviewEventSource, 'name', { value: 'EventSource' });
      window.EventSource = CodeCaptainPreviewEventSource;
    }

    if (typeof window.WebSocket === 'function') {
      const NativeWebSocket = window.WebSocket;
      function CodeCaptainPreviewAppWebSocket(url, protocols) {
        const nextUrl = proxiedWebSocketUrl(String(url));
        return arguments.length === 1
          ? new NativeWebSocket(nextUrl)
          : new NativeWebSocket(nextUrl, protocols);
      }
      CodeCaptainPreviewAppWebSocket.prototype = NativeWebSocket.prototype;
      Object.setPrototypeOf(CodeCaptainPreviewAppWebSocket, NativeWebSocket);
      Object.defineProperty(CodeCaptainPreviewAppWebSocket, 'name', { value: 'WebSocket' });
      window.WebSocket = CodeCaptainPreviewAppWebSocket;
    }
  };
  ...
  const sendHover = (event) => {
    ...
    pendingHover = event;
    if (window.__codecaptainPreviewHoverFrame) return;
    window.__codecaptainPreviewHoverFrame = window.requestAnimationFrame(() => {
      window.__codecaptainPreviewHoverFrame = 0;
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!data || data.source !== 'codecaptain-preview-parent' || data.version !== VERSION) return;
...
export const normalizeProxyTargetUrl = (rawUrl, { allowExternal = false } = {}) => {
  ...
  // Only keep origin here; the proxy path is preserved on the CodeCaptain side.
...
const appendProxyAuthToProxyUrl = (value, { previewToken = '', urlAuthToken = '' } = {}) => {
  ...
  try {
    const parsed = new URL(value, 'http://codecaptain-preview.local');
...
export const createPreviewProxyRuntime = ({
  ...
  const attach = (app, {
    ...
    const injectPreviewBridge = (bodyText, targetOrigin, bridgeNonce) => {
      ...
      const targetOriginScript = `<script${nonceAttr}>window.__codecaptainPreviewTargetOrigin=${JSON.stringify(targetOrigin || '')};</script>`;
    ...
    const proxy = createProxyMiddleware({
      ...
      on: {
        proxyReq: (proxyReq, req) => {
          ...
          // Keep local dev servers from receiving CodeCaptain credentials.
          proxyReq.removeHeader('cookie');
          proxyReq.removeHeader('authorization');
          proxyReq.removeHeader('x-codecaptain-ui-session');
        ...
        proxyRes: responseInterceptor(async (responseBuffer, proxyRes, req, res) => {
          ...
          // Allow the dev server response to be framed inside CodeCaptain even
