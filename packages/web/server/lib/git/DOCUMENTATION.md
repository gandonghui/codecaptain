...
- Fast-create background failures remove CodeCaptain-core sandbox metadata for directories that never became Git worktrees, and remove the pre-created directory only if it is still empty. User-created files are never recursively deleted by this cleanup.
...
- Branches created for new worktrees use `codecaptain/<worktree-name>` pattern.
