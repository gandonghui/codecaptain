...
const STORAGE_DIR = CODECAPTAIN_DATA_DIR;
const STORAGE_FILE = path.join(STORAGE_DIR, 'github-auth.json');
const SETTINGS_FILE = path.join(CODECAPTAIN_DATA_DIR, 'settings.json');
...
function writeJsonFile(payload) {
  ...
  // Atomic write so multiple CodeCaptain instances can safely share the same file.
...
export function getGitHubClientId() {
  const raw = process.env.CODECAPTAIN_GITHUB_CLIENT_ID;
...
export function getGitHubScopes() {
  const raw = process.env.CODECAPTAIN_GITHUB_SCOPES;
