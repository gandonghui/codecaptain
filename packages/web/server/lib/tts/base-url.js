...
export const normalizeCustomOpenAIBaseURL = (value) => {
  ...
  const isDesktop = (process.env.CODECAPTAIN_RUNTIME || '').trim().toLowerCase() === 'desktop';
  const envFlagRaw = process.env.CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  if (!allowRemote && !isAllowedLocalHost(parsed.hostname)) {
    return {
      error: 'Remote custom server URLs are disabled. Set CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS=true to allow this host.',
    };
  }
