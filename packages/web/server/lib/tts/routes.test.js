...
describe('normalizeCustomOpenAIBaseURL', () => {
  const originalRuntime = process.env.CODECAPTAIN_RUNTIME;
  const originalAllowRemote = process.env.CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS;

  afterEach(() => {
    // Restore env vars after each test
    if (originalRuntime === undefined) {
      delete process.env.CODECAPTAIN_RUNTIME;
    } else {
      process.env.CODECAPTAIN_RUNTIME = originalRuntime;
    }
    if (originalAllowRemote === undefined) {
      delete process.env.CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
    } else {
      process.env.CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS = originalAllowRemote;
    }
  });

  it('rejects remote URLs when CODECAPTAIN_RUNTIME is not set (web)', () => {
    delete process.env.CODECAPTAIN_RUNTIME;
    delete process.env.CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  it('allows remote URLs when CODECAPTAIN_RUNTIME is desktop', () => {
  ...
  it('allows remote URLs when CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS is true', () => {
    delete process.env.CODECAPTAIN_RUNTIME;
    process.env.CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'true';
  ...
  it('denies remote URLs on desktop when env var is explicitly false', () => {
    process.env.CODECAPTAIN_RUNTIME = 'desktop';
    process.env.CODECAPTAIN_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'false';
