...
export const createPushRuntime = (deps) => {
  ...
  const resolveVapidSubject = async () => {
    const configured = process.env.CODECAPTAIN_VAPID_SUBJECT;
    if (typeof configured === 'string' && configured.trim().length > 0) {
      return configured.trim();
    }

    const originEnv = process.env.CODECAPTAIN_PUBLIC_ORIGIN;
    if (typeof originEnv === 'string' && originEnv.trim().length > 0) {
      const trimmed = originEnv.trim();
      if (isLoopbackHttpOrigin(trimmed)) {
        return 'mailto:codecaptain@localhost';
      }
      return trimmed;
    }
  ...
  const ensurePushInitialized = async () => {
    ...
    if (subject === 'mailto:codecaptain@localhost') {
      console.warn('[Push] No public origin configured for VAPID; set CODECAPTAIN_VAPID_SUBJECT or enable push once from a real origin.');
    }
