...
describe('local SSE routes', () => {
  it('serves notification SSE with nginx-safe headers', async () => {
    ...
    try {
      ...
      expect(res.body).toContain('codecaptain:notification-stream-ready');
  ...
  it('serves CodeCaptain SSE with nginx-safe headers', () => {
    const { app, getRoute } = createRouteRegistry();
    const clients = new Set();

    registerScheduledTaskRoutes(app, {
      getCodeCaptainEventClients: () => clients,
      writeSseEvent(res, payload) {
        res.write(`data: ${JSON.stringify(payload)}\n\n`);
      },
    });

    const handler = getRoute('GET', '/api/codecaptain/events');
    const req = createMockRequest();
    const res = createMockResponse();
    ...
    expect(res.getHeader('x-accel-buffering')).toBe('no');
    expect(res.flushed).toBe(true);
    expect(res.body).toContain('codecaptain:event-stream-ready');
    expect(clients.has(res)).toBe(true);
