...
import { registerCodeCaptainRoutes } from './lib/opencode/codecaptain-routes.js';
...
const DESKTOP_NOTIFY_PREFIX = '[CodeCaptainDesktopNotify] ';
const uiNotificationClients = new Set();
const uiNotificationWsClients = new Set();
const uiCodeCaptainEventClients = new Set();
...
const shouldSkipApiCompression = () => {
  if (isEnvFlagEnabled(process.env.CODECAPTAIN_SKIP_API_COMPRESSION)) return true;
  if (isEnvFlagEnabled(process.env.CODECAPTAIN_COMPRESS_API)) return false;
  if (isEnvFlagDisabled(process.env.CODECAPTAIN_COMPRESS_API)) return true;
  return process.env.CODECAPTAIN_RUNTIME === 'desktop';
};

const CODECAPTAIN_VERBOSE_REQUEST_LOGS = isEnvFlagEnabled(process.env.CODECAPTAIN_VERBOSE_REQUEST_LOGS);
...
const CODECAPTAIN_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'codecaptain');
const CODECAPTAIN_USER_THEMES_DIR = path.join(CODECAPTAIN_USER_CONFIG_ROOT, 'themes');
const CODECAPTAIN_PROJECTS_CONFIG_DIR = path.join(CODECAPTAIN_USER_CONFIG_ROOT, 'projects');
...
const themeRuntime = createThemeRuntime({
  fsPromises,
  path,
  themesDir: CODECAPTAIN_USER_THEMES_DIR,
...
const CODECAPTAIN_DATA_DIR = process.env.CODECAPTAIN_DATA_DIR
  ? path.resolve(process.env.CODECAPTAIN_DATA_DIR)
  : path.join(os.homedir(), '.config', 'codecaptain');
const SETTINGS_FILE_PATH = path.join(CODECAPTAIN_DATA_DIR, 'settings.json');
const PUSH_SUBSCRIPTIONS_FILE_PATH = path.join(CODECAPTAIN_DATA_DIR, 'push-subscriptions.json');
const REMOTE_CLIENTS_FILE_PATH = path.join(CODECAPTAIN_DATA_DIR, 'remote-clients.json');
const CLOUDFLARE_MANAGED_REMOTE_TUNNELS_FILE_PATH = path.join(CODECAPTAIN_DATA_DIR, 'cloudflare-managed-remote-tunnels.json');
const CLOUDFLARE_LEGACY_NAMED_TUNNELS_FILE_PATH = path.join(CODECAPTAIN_DATA_DIR, 'cloudflare-named-tunnels.json');
...
const projectConfigRuntime = createProjectConfigRuntime({
  fsPromises,
  path,
  projectsDirPath: CODECAPTAIN_PROJECTS_CONFIG_DIR,
});
...
const hmrStateRuntime = createHmrStateRuntime({
  ...
  os,
  processLike: process,
  stateKey: '__codecaptainHmrState',
});
...
const ENV_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START === 'true' ||
                                    process.env.CODECAPTAIN_SKIP_OPENCODE_START === 'true';
const ENV_DESKTOP_NOTIFY = (() => {
  if (process.env.CODECAPTAIN_DESKTOP_NOTIFY === 'true') {
    return true;
  }

  if (process.env.CODECAPTAIN_RUNTIME === 'desktop') {
    return true;
  }

  const argv0 = typeof process.argv?.[0] === 'string' ? process.argv[0] : '';
  const argv1 = typeof process.argv?.[1] === 'string' ? process.argv[1] : '';
  return /codecaptain-server/i.test(argv0) || /codecaptain-server/i.test(argv1);
})();
...
const ENV_CONFIGURED_API_PREFIX = normalizeApiPrefix(
  process.env.OPENCODE_API_PREFIX || process.env.CODECAPTAIN_API_PREFIX || ''
);
...
const processForwardedEventPayload = (payload, emitSyntheticEvent) => {
  ...
  emitSyntheticEvent({
    type: 'codecaptain:session-status',
    properties: {
      sessionID: sessionId,
      status,
      timestamp: Date.now(),
  ...
  emitSyntheticEvent({
    type: 'codecaptain:session-activity',
    properties: {
      sessionId,
...
const scheduledTasksRuntime = createScheduledTasksRuntime({
  ...
  emitTaskRunEvent: (event) => {
    for (const client of uiCodeCaptainEventClients) {
      try {
        writeSseEvent(client, {
          type: 'codecaptain:scheduled-task-ran',
          properties: {
        ...
        uiCodeCaptainEventClients.delete(client);
      }
    }
  },
  logger: console,
...
async function main(options = {}) {
  ...
  const effectiveBindHost = host
    || (typeof process.env.CODECAPTAIN_HOST === 'string' && process.env.CODECAPTAIN_HOST.trim().length > 0
      ? process.env.CODECAPTAIN_HOST.trim()
      : '127.0.0.1');
  const uiPassword = typeof options.uiPassword === 'string'
    ? options.uiPassword
    : (typeof process.env.CODECAPTAIN_UI_PASSWORD === 'string' ? process.env.CODECAPTAIN_UI_PASSWORD : null);
  if (
  ...
  const apiOnly = options.apiOnly === true || isEnvFlagEnabled(process.env.CODECAPTAIN_API_ONLY);
  ...
  console.log(`Starting CodeCaptain on port ${port === 0 ? 'auto' : port}`);

  const sayTTSCapability = await detectSayTtsCapability(process);

  const app = express();
  const serverStartedAt = new Date().toISOString();
  const packagedClientOrigins = new Set(['codecaptain-ui://app']);
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    process,
    codecaptainVersion: CODECAPTAIN_VERSION,
    runtimeName: process.env.CODECAPTAIN_RUNTIME || 'web',
    serverStartedAt,
    gracefulShutdown,
    getHealthSnapshot: () => {
    ...
    verboseRequestLogs: CODECAPTAIN_VERBOSE_REQUEST_LOGS,
    uiPassword,
    tunnelAuthController,
    ...
    server,
    __dirname,
    codecaptainDataDir: CODECAPTAIN_DATA_DIR,
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    codecaptainDataDir: CODECAPTAIN_DATA_DIR,
    codecaptainUserConfigRoot: CODECAPTAIN_USER_CONFIG_ROOT,
    normalizeDirectoryPath,
    resolveProjectDirectory,
    ...
    projectConfigRuntime,
    scheduledTasksRuntime,
    getCodeCaptainEventClients: () => uiCodeCaptainEventClients,
    writeSseEvent,
  });
