...
[CodeCaptain-core](https://opencode.ai) AI coding agent, right inside your editor. No tab-switching, no context loss.

![VS Code Extension](https://github.com/btriapitsyn/codecaptain/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/btriapitsyn/codecaptain) with even more features.**
...
Plus everything from the shared CodeCaptain UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `CodeCaptain: Focus Chat` | Focus the chat panel |
| `CodeCaptain: New Session` | Start a new chat session |
| `CodeCaptain: Open Sidebar` | Open the CodeCaptain sidebar |
| `CodeCaptain: Open Agent Manager` | Launch parallel multi-model runs |
| `CodeCaptain: Open Session in Editor` | Open current or new session in an editor tab |
| `CodeCaptain: Settings` | Open extension settings |
| `CodeCaptain: Restart API Connection` | Restart the CodeCaptain-core API process |
| `CodeCaptain: Show CodeCaptain-core Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **CodeCaptain** submenu:
...
|---------|---------|-------------|
| `codecaptain.apiUrl` | _(empty)_ | URL of an external CodeCaptain-core API server. Leave empty to auto-start a local instance. |
| `codecaptain.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |

## Requirements

- [CodeCaptain-core CLI](https://opencode.ai) installed and available in PATH (or set `OPENCODE_BINARY` env var)
- VS Code 1.85+
...
- `CODECAPTAIN_VSCODE_BIN=cursor bun run vscode:dev`
- `CODECAPTAIN_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/codecaptain-*.vsix`
