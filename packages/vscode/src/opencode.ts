...
function readCodeCaptainSettings(): Record<string, unknown> {
  const settingsPath = path.join(os.homedir(), '.config', 'codecaptain', 'settings.json');
  try {
...
function createConfiguredOpencodeBinaryError(raw: string, normalized: string): Error {
  const messageSuffix = 'CodeCaptain needs the standalone opencode CLI. Install it and set codecaptain.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
...
function validateConfiguredOpencodeBinaryForManagedStart(): string | null {
  ...
  try {
    const settings = readCodeCaptainSettings();
...
function resolveOpencodeCliPath(): string | null {
  ...
  if (sharedFromCodeCaptain && isExecutable(sharedFromCodeCaptain) && !isMacOpenCodeAppBundlePath(sharedFromCodeCaptain)) {
    return sharedFromCodeCaptain;
  }
...
async function waitForReady(
  ...
  const outputChannel = vscode.window.createOutputChannel('CodeCaptainManager');
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    const onExit = (code: number | null) => {
      ...
      const appBundleHint = isMacOpenCodeAppBundlePath(binary)
        ? ' The configured binary appears to point at the macOS desktop app bundle; CodeCaptain needs the standalone opencode CLI.'
        : '';
