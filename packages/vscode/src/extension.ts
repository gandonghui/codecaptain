...
const SETTINGS_KEY = 'codecaptain.settings';
...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('CodeCaptain');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      await vscode.commands.executeCommand('codecaptain.chatView.focus');
      await vscode.commands.executeCommand(moveCommandId);
      return 'moved';
    } catch (error) {
      outputChannel?.appendLine(
        `[CodeCaptain] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  const maybeMoveChatToRightSidebarOnStartup = async () => {
    ...
    const attempted = context.globalState.get<boolean>('codecaptain.sidebarAutoMoveAttempted') || false;
    if (attempted) return;
    await context.globalState.update('codecaptain.sidebarAutoMoveAttempted', true);
  ...
  const config = vscode.workspace.getConfiguration('codecaptain');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.openSidebar', async () => {
      // Best-effort: open the container (if available), then focus the chat view.
      try {
        await vscode.commands.executeCommand('workbench.view.extension.codecaptain');
      } catch (e) {
        outputChannel?.appendLine(`[CodeCaptain] workbench.view.extension.codecaptain failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('codecaptain.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[CodeCaptain] codecaptain.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('CodeCaptain: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[CodeCaptain] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('CodeCaptain: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    const opened = await vscode.commands.executeCommand<boolean>('codecaptain.openSidebar');
    if (!opened) {
      return false;
    }
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[CodeCaptain] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('CodeCaptain: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.focusChat', async () => {
      await vscode.commands.executeCommand('codecaptain.chatView.focus');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.internal.settingsSynced', (settings: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.openAgentManager', () => {
      agentManagerProvider?.createOrShow();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.setActiveSession', (sessionId: unknown, title?: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('CodeCaptain: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.openSessionInEditor', (sessionId: string, title?: string) => {
      if (typeof sessionId !== 'string' || sessionId.trim().length === 0) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.openNewSessionInEditor', () => {
      sessionEditorProvider?.createOrShowNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.openCurrentOrNewSessionInEditor', () => {
      if (activeSessionId) {
        sessionEditorProvider?.createOrShow(activeSessionId, activeSessionTitle ?? undefined);
      } else {
        sessionEditorProvider?.createOrShowNewSession();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('CodeCaptain: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('CodeCaptain: Failed to restart API - {0}', String(e)));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.addToContext', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('CodeCaptain [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('CodeCaptain [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('CodeCaptain: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('CodeCaptain: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('CodeCaptain [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('CodeCaptain [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('CodeCaptain [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('CodeCaptain: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`CodeCaptain: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.showSettings', () => {
      chatViewProvider?.showSettings();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('codecaptain.showOpenCodeStatus', async () => {
      const config = vscode.workspace.getConfiguration('codecaptain');
      ...
      const lines = [
        ...
        `CodeCaptain version: ${extensionVersion || '(unknown)'}`,
        ...
        `OpenCode binary (configured): ${(vscode.workspace.getConfiguration('codecaptain').get<string>('opencodeBinary') || '').trim() || '(none)'}`,
