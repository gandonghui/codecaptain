...
export function getWebviewHtml(options: WebviewHtmlOptions): string {
  ...
  // 
  // Logo geometry matches CodeCaptainLogo.tsx:
...
<head>
  ...
  <title>CodeCaptain</title>
</head>
<body>
  <!-- Initial loading screen with simplified CodeCaptain logo -->
  ...
  <script>
    ...
    window.__CODECAPTAIN_HOME__ = "${workspaceFolder.replace(/\\/g, '\\\\')}";
    
    function getBootstrapMessages() {
      var locale = 'en';
      try {
        var rawLocale = window.localStorage.getItem('codecaptain.i18n.v1');
        if (rawLocale) {
  ...
  <script type="module">
    ...
    if (!devServerUrl) {
      ...
      const getDevMessages = () => {
        try {
          const rawLocale = window.localStorage.getItem('codecaptain.i18n.v1');
          if (rawLocale) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[CodeCaptain] VS Code webview dev bundle unavailable, retrying...', error);
