...
# alias; CODECAPTAIN_UI_PASSWORD is the canonical runtime variable.
if [ -z "${CODECAPTAIN_UI_PASSWORD:-}" ] && [ -n "${UI_PASSWORD:-}" ]; then
  CODECAPTAIN_UI_PASSWORD="$UI_PASSWORD"
  export CODECAPTAIN_UI_PASSWORD
fi

if [ -n "${CODECAPTAIN_UI_PASSWORD:-}" ]; then
...
CODECAPTAIN_HOST="${CODECAPTAIN_HOST:-0.0.0.0}"
export CODECAPTAIN_HOST
...
if [ -n "${CODECAPTAIN_UI_PASSWORD:-}" ]; then
  set -- "$@" --ui-password "$CODECAPTAIN_UI_PASSWORD"
fi
"$@"
