#!/usr/bin/env bash
# CodeCaptain Install Script
# Usage: curl -fsSL https://raw.githubusercontent.com/btriapitsyn/codecaptain/main/scripts/install.sh | bash

set -euo pipefail

PACKAGE_NAME="@codecaptain/web"
BIN_NAME="codecaptain"
MIN_NODE_VERSION=22
...
main() {
  ...
  # If CodeCaptain is already installed, hand off to its own updater instead
  # of guessing a package manager. `codecaptain update` detects which manager
  ...
  if command_exists "$BIN_NAME"; then
    info "CodeCaptain is already installed — updating via 'codecaptain update'..."
    echo ""
    if codecaptain update; then
      echo ""
      success "CodeCaptain is up to date!"
      exit 0
    fi
    echo ""
    error "Update failed."
...
EOF
    printf '%b\n' "$NC"
    success "CodeCaptain installed successfully!"
    echo ""
    ...
    if command_exists "$BIN_NAME"; then
      echo "  Get started:"
      echo "    codecaptain              # Start server on port 3000"
      echo "    codecaptain --help       # Show all options"
    else
      ...
      if [ -n "$bin_dir" ]; then
        ...
        echo "  then restart your terminal and run: codecaptain"
      fi
    fi
    echo ""
    echo "  Prerequisites:"
