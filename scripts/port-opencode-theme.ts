...
/*
 * Port OpenCode themes into full CodeCaptain theme JSON files.
 *
 * Usage:
 *   bun run themes:port:opencode --list
 *   bun run themes:port:opencode github cursor lucent-orng --force
 *   bun run themes:port:opencode aura --out-dir /tmp/codecaptain-theme-port-test --force
 ...
 *     or existing CodeCaptain theme JSON files
 *   - writes complete CodeCaptain light/dark theme JSON output with solid core
...
type ThemeSource = DesktopTheme | ContextThemeJson | CodeCaptainTheme;
...
function isCodeCaptainTheme(value: ThemeSource): value is CodeCaptainTheme {
...
function syntheticTokensFromCodeCaptainTheme(theme: CodeCaptainTheme): ResolvedTheme {
...
async function main(): Promise<void> {
  ...
  const generated: Array<{ fileName: string; theme: CodeCaptainTheme }> = [];
    ...
    if (isCodeCaptainTheme(source)) {
      ...
      generated.push({
        ...
        theme: buildTheme(
          syntheticTheme,
          variant,
          syntheticTokensFromCodeCaptainTheme(source),
